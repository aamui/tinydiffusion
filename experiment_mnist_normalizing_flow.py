...
from model_normalizing_flow import LatentNormalizingFlow
...
if __name__ == "__main__":
    ...
    # Pretrain AE
    encoder = Encoder(latent_ch=4)
    decoder = Decoder(latent_ch=4)

    nf_model = LatentNormalizingFlow(latent_ch=4, latent_hw=7)
    nf_model.train_autoencoder(encoder, decoder, X_train, X_test,
                           num_epochs=5, device=device)

    nf_model.train_nf(encoder, X_train, X_test, num_epochs=150, device=device)

    # Sample
    samples = nf_model.generate_with_model(decoder, num_samples=16, device=device)
    visualize_n_samples(samples, n=5, title="LFM Generated")