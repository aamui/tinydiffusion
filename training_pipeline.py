...
def training_pipeline_lfm(model_class, num_train_samples=500000, num_test_samples=100000, num_epochs=50, device='mps', batch_size=512, **kwargs):
    ...
    model = model_class(load_from_path=f'checkpoints/synthetic_lfm_{num_epochs}.pth',
                                encoder_ckpt="checkpoints/synthetic_ae_encoder.pth",
                                decoder_ckpt="checkpoints/synthetic_ae_decoder.pth", **kwargs)

    generated_images = model.generate_dataset(num_samples=500, number_of_steps=100, device=device)
...
def training_pipeline_nf(model_class, num_train_samples=500000, num_test_samples=100000, num_epochs=50, device='mps', batch_size=512, **kwargs):
    ...
    model.train_function(X_train, y_train,X_test, y_test,num_epochs=num_epochs,device=device,model_name=f"eval_syn_{model_class.__name__}")
    ...
    model = model_class(dim=dim,num_classes=num_classes,load_from_path=f"./checkpoints/eval_syn_{model_class.__name__}_epoch_{num_epochs}.pth",  **kwargs)
    ...
    visualize_n_samples(samples_img, n=5, output_binarization=True, file_name=f"eval_syn_{model_class.__name__}_generated_samples.pdf")
